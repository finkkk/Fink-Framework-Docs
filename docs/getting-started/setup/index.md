...
本章节将带你快速安装并导入 Fink Framework 到 Unity 项目中，并完成必要的初始化设置。
...
导入框架前，请确认项目中没有重复安装以下依赖。Fink Framework 已将它们内置在 `Assets/FinkFramework/Plugins/` 目录中。

| 依赖库 | 用途 | 版本 | 备注 |
| --- | --- | --- | --- |
| **UniTask** | 异步加载、任务系统支持 | 源码版 | 必需，提供性能优于协程的 `async/await` |
| **Odin Serializer** | 数据序列化与反射支持 | 源码版 | 必需，数据管线核心依赖 |
| **Newtonsoft.Json** | JSON 解析与调试输出 | DLL | 可选，但推荐保留 |
| **ExcelDataReader** | Excel 配表读取 | DLL | 数据管线必需 |

> **注意：请不要重复安装以上依赖。**

如果项目中已经存在同名依赖（例如 UniTask 或 Newtonsoft.Json），可能出现类型重复、命名空间冲突，或因 DLL 重复导入导致编译失败。

处理建议：

1. 已安装相同依赖时，可以删除 `Assets/FinkFramework/Plugins/` 下对应的那一份。
2. 如果不确定应删除哪一份，建议保留 Fink Framework 自带版本，并移除项目原有版本。
3. 两边版本不同时，优先使用较新版本。

框架内置依赖已按正确顺序组织，不会污染项目结构。
...
- 确认更新后，框架会自动下载对应的 UnityPackage，完成文件校验、旧版本备份和新版本导入；
- 如果更新失败或被中断，更新器会尝试恢复更新前的框架文件。

自动更新只替换 `Assets/FinkFramework` 目录，不会修改框架配置、数据文件或项目中已经生成的业务脚本。不过，直接修改在 `Assets/FinkFramework` 内的源码会被新版本覆盖，更新前请先提交版本控制或自行备份这些改动。

> 欢迎窗口中的手动检查不受自动检查开关和检查间隔限制，随时都可以使用。

### 4.1 手动从 GitHub 更新

如果自动更新不可用，也可以进入 [GitHub Releases](https://github.com/finkkk/Fink-Framework/releases) 下载最新的 `FinkFramework-v版本号.unitypackage`，再手动导入项目。

为避免新版已经删除的旧文件残留，建议先备份项目和自定义改动，再移除旧的 `Assets/FinkFramework` 目录并导入新版本。具体版本如有额外迁移要求，请同时阅读对应 Release 的说明。
...
- 点击欢迎窗口中的“打开框架设置”；
- 点击 `Edit` → `Project Settings` → `Fink Framework` → `Framework`。
...
- **强制关闭 XR / 新输入系统 / URP**：默认关闭，让框架自动检测项目环境。只有安装了相关功能但不希望框架使用时才开启；
...
| UI 渲染模式 | [UI 系统配置](/ui-system/configuration/) |
