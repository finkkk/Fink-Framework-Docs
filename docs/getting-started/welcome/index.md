...
它既提供可直接调用的运行时 API，也提供数据工具、本地化编辑器、UI 面板生成器、项目统计与归档等编辑器工具。框架不替代 Unity，而是统一常用系统的运行时入口、项目配置和编辑器工作流。
...
| 输入系统 | 提供键鼠事件绑定、输入设备类型检测以及 Unity 输入后端的统一设备活动识别。 | [输入系统](/input-system/) |
| 运行时基础设施 | 提供单例、对象池、事件绑定、生命周期代理、计时器、音频和场景切换等常用能力。 | [基础系统](/core-systems/) |
...
- **全局设置**：集中管理脚本根目录、模块开关、环境检测、UI 模式、数据模式、加密和资源后端；
...
- `InputManager` 负责基于 Unity `Input` 的键鼠事件绑定；`DeviceDetectionManager` 根据框架最终启用的输入后端识别键鼠、手柄与触摸设备；
